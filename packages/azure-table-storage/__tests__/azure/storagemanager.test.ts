...
describe('azure-storage-manager-tests', () => {
    ...
    test('cache cleanup tests', (done: any) => {
        ...
        let newCarIdentifier: AzureDocumentIdentifier = AzureDocumentIdentifier.fromObj(newCar);
        let newCarIdentifier2: AzureDocumentIdentifier = AzureDocumentIdentifier.fromObj(newCar2);

        let cache: AzureTableDocumentCacheInMemory<CarTest> = new AzureTableDocumentCacheInMemory<CarTest>(true, moment.duration(5, 'days'));
